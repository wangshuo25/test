...
%>	
<div class="checkbox checkbox-info"><input type="checkbox" id="d${status.index}" name="dimval" desc="${e.name}" value="${e.id}" <%if(cn.zup.bis.util.RSBIUtils.exist(id, ids.split(","))){%>checked="true"<%}%> ><label for="d${status.index}">${e.name}</label></div>
<%
}
%>
</c:forEach>
