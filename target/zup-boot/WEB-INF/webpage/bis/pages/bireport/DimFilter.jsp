...
<c:choose>
	...
	<div title="值筛选" <c:if test="${filtertype == 2}">data-options="selected:true"</c:if>>
			...
			%>	
			<div class="checkbox checkbox-info"> <input type="checkbox" id="d${statu.index}" name="dimval" value="${e.id}" <%if(cn.zup.bis.util.RSBIUtils.exist(id, ids.split(","))){%>checked="true"<%}%> > <label for="d${statu.index}"> ${e.name} <c:if test="${dimType == 'day'}"><!-- 追加日期的节日 -->
			<%=cn.zup.bis.util.RSBIUtils.getFestival(id, request)%>
			</c:if></label></div>
			<%
			}
			%>
