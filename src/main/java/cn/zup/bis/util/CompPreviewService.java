...
//import com.ruisi.ext.service.ExtLoginInfoLoader;
...
public class CompPreviewService {
	...
	private com.ruisi.ext.engine.wrapper.ExtRequest request;
	...
	public CompPreviewService(HttpServletRequest req, HttpServletResponse resp, ServletContext sctx) throws IOException{
		request = (ExtRequest) new ExtRequestImpl(req);
		response = (ExtResponse) new ExtResponseImpl(resp);
		context = sctx;
		dao = (DaoHelper) DaoUtils.getDaoHelper(context);
	...
	public void initPreview() throws FileUploadException {
		...
		InputOption option = (InputOption) InputOptionFactory.createInputOption(request, (com.ruisi.ext.engine.wrapper.ExtResponse) response, params);
		...
		String fieldLoader = "cn.zup.bis.MyCrossFieldLoader";
		if(fieldLoader == null){
			throw new ExtRuntimeException("未配置 fieldLoader.");
		}
		try {
			loader = (CrossFieldLoader)Class.forName(fieldLoader).newInstance();
			loader.setRequest((com.ruisi.ext.engine.wrapper.ExtRequest) this.request);
		} catch (Exception e) {
