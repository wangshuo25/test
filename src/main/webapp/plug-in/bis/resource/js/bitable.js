			...
			**/
			...
			if(node.attributes.col_type == 2 && $(this).attr("id") == "d_kpi"){
				...
				if(!kpiExist(node.attributes.col_id, json.kpiJson)){
					json.kpiJson.push({"kpi_id":node.attributes.col_id, "kpi_name" : node.text, "col_name":node.attributes.col_name, "aggre":node.attributes.aggre, "alias":node.attributes.alias,"tname":node.attributes.tname,"unit":node.attributes.unit});
				}else{
					msginfo("度量已经存在。");
					return;
				}
			...
			if(node.attributes.col_type == 1){
				...
				if($(this).attr("id") == "d_colDims"){
					...
					json.cols.push({"id":node.attributes.col_id, "dimdesc" : node.text, "type":node.attributes.dim_type, "colname":node.attributes.col_name,"alias":node.attributes.alias,"tname":node.attributes.tname, "grouptype":node.attributes.grouptype,"valType":node.attributes.valType});
				...
				if($(this).attr("id") == "d_rowDims"){
					...
					json.rows.push({"id":node.attributes.col_id, "dimdesc" : node.text, "type":node.attributes.dim_type, "colname":node.attributes.col_name,"alias":node.attributes.alias,"tname":node.attributes.tname,"grouptype":node.attributes.grouptype,"valType":node.attributes.valType});
...
function tableView(table, compId){
	...
	var json = {cols:table.cols,rows:table.rows,kpiJson:table.kpiJson,dsid:table.dsid, dsetId:table.dsetId,};
	__showLoading();

	$.ajax({
	   type: "POST",
	   url: "/rest/bis/bireport/TableController/TableView",
	   dataType:"html",
	...
	$('#pdailog').dialog({
		...
		buttons:[{
			...
			handler:function(){
				...
				var pic1 = $("#pdailog #w1").attr("class");//样式css
				...
				kpi.warning = {pictype:pictype,logic1:logic1,val1:val1,logic2:logic2,val2:val2,pic1:pic1,pic2:pic2,pic3:pic3}
				tableView(comp, compId);
				$('#pdailog').dialog('close');
			}
		},{
