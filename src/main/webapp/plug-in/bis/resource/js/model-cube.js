...
function initcubeTable(){
	...
	border:false,
	fit:true,
	url:'rest/bis/CubeController/listCube',
		toolbar:[{
			text:'新增',
			iconCls:'icon-add2',
			handler:function(){
		...
		columns:[[
			{field:'cubeId',title:'标识',width:100,align:'center'},
			{field:'cubeName',title:'立方体名称',width:200,align:'center'},
			{field:'desc',title:'立方体说明',width:180,align:'center'},
			{field:'dsetName',title:'数据集',width:180,align:'center'},

			{field:'action',title:'操作',width:200,align:'center',
				formatter:function(value,row,index) {

					return "<button href='javascript:void(0)' onclick='delCube(&apos;" + index+ "&apos;)'><img src='plug-in/bis/resource/jquery-easyui-1.4.4/themes/icons/no.png' >删除</button><button href='javascript:void(0)' style='margin-left:10px;' onclick='newCube(&apos;" + true+ " &apos;,&apos;" + index+ "&apos;)'><img src='plug-in/bis/resource/jquery-easyui-1.4.4/themes/icons/pencil.png' >编辑</button>";
				}
			}
...
function delCube(a){
	var rows= $("#cubetable").datagrid("getRows");
	var id=rows[a].cubeId;
	if(confirm("是否确认删除？")){
		$.ajax({
			type:"POST",
			url:"rest/bis/CubeController/delCube",
			dataType:"json",
			data:{cubeId:id},
			success:function(){
...
function newCube(isupdate, a){
	//清空 delObj 对象， 这个对象用在编辑时，判断哪些内容被删除了。
	window.delObj = [];
	var cube;
	if(isupdate){
		var rows= $("#cubetable").datagrid("getRows");
		var cubeId=rows[a].cubeId;
		$.ajax({
			type:"POST",
			url:"rest/bis/CubeController/getCube",
			async: false,
			dataType:"JSON",
			data:{cubeId:cubeId},
	...
	//初始化左边树形状
	var leftTreeView = function(dset, cube){//传入的是数据集的cfg
		 // var cld = [];
		 // var dt = [{id:'leftroot',text:dset.master,iconCls:'icon-table', children:cld}];
		  ...
		  for(i=0; i<c.cols.length; i++){
			  var r = c.cols[i];
			  if(!tabExist(r.tname)){//每个表进入集合一次即可
				tbs.push(r.tname);//把所有数据集字段对应的表，全都选出来
			  }
		  }
		  var findcols = function(tname){
			  ...
			  for(j=0; j<c.cols.length; j++){
				  if(c.cols[j].tname == tname){
					  ret.push(c.cols[j]);//数据集中某个表的字段，全都选出来
				  }
			  }
			  return ret;
		  }
		  ...
		  for(i=0; i<tbs.length; i++){
			  ...
			  dt.push(nd);	  //获取表
			  var cols = findcols(tbs[i]);//根据表找字段
			  for(l=0; l<cols.length; l++){
				   var r = cols[l];
				  var node = {id:r.name,text:r.name,iconCls:'icon-datasetcol2',attributes:{tp:'node', vtype:r.type, col:r.name, tname:r.tname, expression:r.expression}}
				  ccld.push(node);
			  }
		  }
		  // //动态字段
				   ...
				   //隐藏已选择的维度和指标 列
					...
					window.setTimeout(function(){
							...
							for(j=0; j<roots.length; j++){
								var r = roots[j];
								nodes = nodes.concat($("#cubelefttree").tree("getChildren", r.target));//获取所有的子节点
							}
...
function initRightCubeTree(cube){
	...
	if(cube){ //给立方体添加维度及指标
		...
		for(i=0; i<cube.dims.length; i++){
			var d = cube.dims[i];
			var obj = {id:d.id, text:d.text, attributes:{tp:"dim",drag:true,col:d.col_name,tname:d.tname,dispName:d.text,vtype:d.valType,alias:d.alias, dimtype:d.dim_type,calc:false, targetId:d.col_id },iconCls:"icon-dim"};
//id，记录的id
//text,别名
//col, 维度的名称（指标带着（））
//tname,表名
//dispName，别名
//vtype，string?  int?
//alias，维度的真名 注意区分"col_name
//targetId,如果没有组则为维度的id，有组则是组的id

			if(d.grouptype != "" && d.grouptype != null){
				var group = groupexist(d.grouptype);//查看是不是已经有了分组的节点
				if(group == null){//若没有则先建立，再添加
				...
				targdt[0].children[0].children.push(obj);//不存在组，则树直接生成维度
			}
		}
		...
		for(i=0; i<cube.kpis.length; i++){
			...
			var col = k.alias;
			kpis.push({id:k.id, text:k.aggre+'('+k.text+")",attributes:{tp:"kpi",drag:true,aggre:k.aggre,col:col,tname:k.tname, unit:(k.unit==null?"":k.unit),  dispName:k.text, alias:k.alias,targetId:k.colid},iconCls:"icon-kpi"});
			// id,在立方体内的指标id
			// text,sum(**)
			// aggre,计算方法sum、count
			// col,指标的真名 不带sum
...
function ds2cube(){
	...
	if(right.text == '度量' || parent.text == '度量'){
		...
		var o = {id:cid, text:'sum('+left.text+")",attributes:{tp:"kpi",drag:true,aggre:"sum",col:left.attributes.col, tname:left.attributes.tname,dispName:left.text,alias:left.id},iconCls:"icon-kpi"};
		if(right.text == '度量'){
		...
		var o = {id:cid, text:left.text, attributes:{tp:"dim",drag:true,col:left.attributes.col,dispName:left.text,tname:left.attributes.tname,vtype:left.attributes.vtype,alias:left.attributes.col},iconCls:"icon-dim", targetId:""};  //通过targetId 来指引对应数据库的的字段 ID, 用在修改上
...
function editcubecol(){
	...
	if(right.attributes.tp == 'dim'){
		...
		ctx = "<div class=\"textpanel\"><span class=\"inputtext\">度量字段：</span>"+right.attributes.col+"<br/><span class=\"inputtext\">显示名称：</span><input type=\"text\" id=\"kpiname\" name=\"kpiname\" class=\"inputform2\" value=\""+right.attributes.dispName+"\"><br/>"
		+ "<span class=\"inputtext\">计算方式：</span><select id=\"kpiaggre\" name=\"kpiaggre\" class=\"inputform2\">"+tpstr+"</select> <br>"
		+ "<span class=\"inputtext\">度量单位：</span><input type=\"text\" id=\"kpiunit\" name=\"kpiunit\" class=\"inputform2\" value=\""+(right.attributes.unit?right.attributes.unit:"")+"\"> <br></div>";
	...
	$('#dsColumn_div').dialog({
		...
		width: 350,
		height:  right.attributes.tp == 'group' ? 150 :240,
		closed: false,
		cache: false,
		modal: true,
		toolbar:null,
...
function cube2ds(){
	...
	if(right.attributes.tp != 'group'){ //分组删除不用关联左边树
		...
		for(i=0; i<ls.length; i++){
			var c = ls[i].children;
			for(j=0; j<c.length; j++){
				cld.push(c[j]);//获得所有的孩子
			}
		}
		for(i=0; i<cld.length; i++){
			if(cld[i].id == id){//找到右边要移除的孩子
...
function savecubecfg(pageJson, update){
	...
	for(i=0; i<dims.length; i++){
		...
		if(d.attributes.tp == "group"){
			...
			var obj = {name:d.attributes.dispName, type:d.attributes.dimtype,col:d.attributes.col, tname:d.attributes.tname, alias:d.attributes.alias, vtype: d.attributes.vtype, calc:0,isupdate:d.attributes.isupdate};
	...
	for(i=0; i<kpis.length; i++){
		var t = kpis[i];
		cubeKpi.push({name:t.attributes.dispName,col:t.attributes.col,tname:t.attributes.tname,alias:t.attributes.alias,fmt:t.attributes.fmt,unit:t.attributes.unit,aggre:t.attributes.aggre,kpinote:t.attributes.kpinote,calc:0,isupdate:t.attributes.isupdate});
	}
	...
	if(update){
		$.ajax({
			type:"POST",
			url:"rest/bis/CubeController/updateCube",
		...
		$.ajax({
			...
			type:"POST",
			url:"rest/bis/CubeController/saveCube",
			dataType:"HTML",
			contentType : "application/json",   
			data:json,
			success:function(resp){
